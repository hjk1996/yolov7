...
def detect(save_img=False):
    ...
    old_img_width = old_img_height = img_size
    old_img_batch = 1

    t0 = time.time()
    for path, img, cv2_img, vid_cap in dataset:
        img = torch.from_numpy(img).to(device)
        img = img.half() if half else img.float()  # uint8 to fp16/32
        # 이미지 정규화
        img /= 255.0  # 0 - 255 to 0.0 - 1.0
        # 이미지가 3차원이라면 맨 앞에 차원을 더해줌
        if img.ndimension() == 3:
            img = img.unsqueeze(0)

        # Warmup
        if device.type != 'cpu' and (old_img_batch != img.shape[0] or old_img_height != img.shape[2] or old_img_width != img.shape[3]):
            old_img_batch = img.shape[0]
            old_img_height = img.shape[2]
            old_img_width = img.shape[3]
            for i in range(3):
        ...
        # Apply Classifier
        if classify:
            pred = apply_classifier(pred, modelc, img, cv2_img)

        # Process detections
        for i, det in enumerate(pred):  # detections per image
            if webcam:  # batch_size >= 1
                p, s, im0, frame = path[i], '%g: ' % i, cv2_img[i].copy(), dataset.count
            else:
                p, s, im0, frame = path, '', cv2_img, getattr(dataset, 'frame', 0)
